...
template <typename Value_, bool IsMask_, typename Derived_> struct ArrayBaseT {
    ...
    /// Are elements of this array implemented using the HIP (AMD) backend?
    static constexpr bool IsHIP = is_hip_v<Value_>;

    /// Are elements of this array implemented using a JIT-compiled backend?
    static constexpr bool IsJIT = IsLLVM || IsCUDA || IsMetal || IsHIP;
